...
void EnergyFunctional::getImuHessianCurrentFrame(int fi, CalibHessian *HCalib,
  ...
  FrameHessian *pprv_fh = fi >= 2 ? frames[fi - 2]->data : nullptr;
  int cur_idx = CPARS + 3 + 29 * fi;
  int prv_idx = CPARS + 3 + 29 * (fi - 1);
  int pprv_idx = CPARS + 3 + 29 * (fi - 2);

  spline_valid = (cur_fh->shell->trackingRef == prv_fh->shell) &&
                 ((cur_fh->shell->timestamp - prv_fh->shell->timestamp) <
                  setting_maxImuInterval);
  bool vel_valid = pprv_fh && (prv_fh->shell->trackingRef == pprv_fh->shell) &&
                   ((prv_fh->shell->timestamp - pprv_fh->shell->timestamp) <
                    setting_maxImuInterval);
  ...
  if (spline_valid) {
    ...
    // velocity
    if (vel_valid) {
      double tppf = pprv_fh->shell->timestamp - prv_fh->shell->timestamp;
      assert(tppf < 0);
      double tppf2 = tppf * tppf;
      r_cst.segment<3>(3) =
          (1 / tpf) * (prv_fh->PRE_camToWorld.translation() -
                       cur_fh->PRE_camToWorld.translation()) -
          (1 / tppf) * (pprv_fh->PRE_camToWorld.translation() -
                        prv_fh->PRE_camToWorld.translation()) +
          (tppf * prv_fh->spline_q + tppf2 * prv_fh->spline_c +
           tpf * cur_fh->spline_q + 2 * tpf2 * cur_fh->spline_c)
              .head(3);
      J_cst.block<3, 3>(3, pprv_idx) = -SCALE_XI_TRANS / tppf * I33;
      J_cst.block<3, 3>(3, prv_idx) =
          SCALE_XI_TRANS * (1 / tpf + 1 / tppf) * I33;
      J_cst.block<3, 3>(3, cur_idx) = -SCALE_XI_TRANS / tpf * I33;
      J_cst.block<3, 3>(3, prv_idx + 17) = SCALE_SQ_TRANS * tppf * I33;
      J_cst.block<3, 3>(3, prv_idx + 23) = SCALE_SC_TRANS * tppf2 * I33;
      J_cst.block<3, 3>(3, cur_idx + 17) = SCALE_SQ_TRANS * tpf * I33;
      J_cst.block<3, 3>(3, cur_idx + 23) = SCALE_SC_TRANS * 2 * tpf2 * I33;
    }
...
void EnergyFunctional::solveSystemF(int iteration, double lambda,
  ...
  if (imu_valid) {
    ...
    MatXX H_imu;
    VecX b_imu;
    getImuHessian(H_imu, b_imu, J_cst, r_cst, HCalib, is_spline_valid,
                  iteration == 0);
