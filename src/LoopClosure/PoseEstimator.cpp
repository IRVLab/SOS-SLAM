...
PoseEstimator::PoseEstimator(int ww, int hh) : refAffGToL(0, 0) {
  // warped buffers
  bufWarped_idepth = allocAligned<4, float>(ww * hh, ptrToDelete);
  bufWarped_u = allocAligned<4, float>(ww * hh, ptrToDelete);
  bufWarped_v = allocAligned<4, float>(ww * hh, ptrToDelete);
  bufWarped_dx = allocAligned<4, float>(ww * hh, ptrToDelete);
  bufWarped_dy = allocAligned<4, float>(ww * hh, ptrToDelete);
  bufWarped_residual = allocAligned<4, float>(ww * hh, ptrToDelete);
  bufWarped_weight = allocAligned<4, float>(ww * hh, ptrToDelete);
  bufWarped_ref_color = allocAligned<4, float>(ww * hh, ptrToDelete);
...
void PoseEstimator::calcGSSSE(int lvl, Mat88 &H_out, Vec8 &b_out,
  ...
  __m128 b0 = _mm_set1_ps(refAffGToL.b);
  __m128 a = _mm_set1_ps((float)(AffLight::fromToVecExposure(
      refAbExposure, newFrame->ab_exposure, refAffGToL, aff_g2l)[0]));
  ...
  assert(n % 4 == 0);
  for (int i = 0; i < n; i += 4) {
    __m128 dx = _mm_mul_ps(_mm_load_ps(bufWarped_dx + i), fxl);
    __m128 dy = _mm_mul_ps(_mm_load_ps(bufWarped_dy + i), fyl);
    __m128 u = _mm_load_ps(bufWarped_u + i);
    __m128 v = _mm_load_ps(bufWarped_v + i);
    __m128 id = _mm_load_ps(bufWarped_idepth + i);
    ...
    acc.updateSSE_eighted(
        ...
        _mm_mul_ps(a, _mm_sub_ps(b0, _mm_load_ps(bufWarped_ref_color + i))),
        minusOne, _mm_load_ps(bufWarped_residual + i),
        _mm_load_ps(bufWarped_weight + i));
  }
...
Vec6 PoseEstimator::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l,
  ...
  Vec2f affLL = AffLight::fromToVecExposure(
                    refAbExposure, newFrame->ab_exposure, refAffGToL, aff_g2l)
                    .cast<float>();
  ...
  for (size_t i = 0; i < pts.size(); i++) {
    float x = pts[i].first(0);
    float y = pts[i].first(1);
    float z = pts[i].first(2);
    float u0 = x / z;
    float v0 = y / z;
    ...
    float refColor = pts[i].second[lvl];
    ...
    if (fabs(residual) > cutoffTH) {
      ...
      bufWarped_idepth[numTermsInWarped] = new_idepth;
      bufWarped_u[numTermsInWarped] = u;
      bufWarped_v[numTermsInWarped] = v;
      bufWarped_dx[numTermsInWarped] = hitColor[1];
      bufWarped_dy[numTermsInWarped] = hitColor[2];
      bufWarped_residual[numTermsInWarped] = residual;
      bufWarped_weight[numTermsInWarped] = hw;
      bufWarped_ref_color[numTermsInWarped] = refColor;
      numTermsInWarped++;
    }
  }

  while (numTermsInWarped % 4 != 0) {
    bufWarped_idepth[numTermsInWarped] = 0;
    bufWarped_u[numTermsInWarped] = 0;
    bufWarped_v[numTermsInWarped] = 0;
    bufWarped_dx[numTermsInWarped] = 0;
    bufWarped_dy[numTermsInWarped] = 0;
    bufWarped_residual[numTermsInWarped] = 0;
    bufWarped_weight[numTermsInWarped] = 0;
    bufWarped_ref_color[numTermsInWarped] = 0;
    numTermsInWarped++;
  }
  bufWarped_n = numTermsInWarped;
...
bool PoseEstimator::estimate(const LoopFrame *cur_frame,
  ...
  for (int lvl = coarsest_lvl; lvl >= 0; lvl--) {
    ...
    if (DEBUG_PRINT) {
      Vec2f relAff =
          AffLight::fromToVecExposure(refAbExposure, newFrame->ab_exposure,
                                      refAffGToL, aff_g2l_current)
              .cast<float>();
